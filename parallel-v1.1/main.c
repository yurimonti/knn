...
int main(int argc, char *argv[])
{
    ...
    if (my_rank == 0)
    {
        ...
        {
            MPI_Send(points + (i * points_per_process), points_per_process, point_type, i, 0, MPI_COMM_WORLD);
            MPI_Send(points + (i * points_per_process), points_per_process, point_type, i, 1, MPI_COMM_WORLD);
        }
    }
    else
    {
        MPI_Recv(my_points, points_per_process, point_type, 0, 0, MPI_COMM_WORLD, &status);
    ...
    for (i = 0; i < num_procs; i++)
    {
        if (i != 0)
        {
            ...
            MPI_Bsend(received_points, points_per_process, point_type, (my_rank + i) % num_procs, 1, MPI_COMM_WORLD);
            MPI_Recv(received_points, points_per_process, point_type, (my_rank - i + num_procs) % (num_procs), 1, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
